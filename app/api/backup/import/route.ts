...
const DATE_FIELDS: Record<Collection, string[]> = {
  ...
  activityPeriods: ["createdAt"],
  levelMeetings: ["createdAt", "updatedAt"],
  teachers: ["createdAt", "updatedAt"],
  levelMeetingTeachers: ["createdAt"],
...
function delegate(tx: Prisma.TransactionClient, collection: Collection): AnyDelegate {
  const map: Record<Collection, unknown> = {
    ...
    activityPeriods: tx.activityPeriod,
    levelMeetings: tx.levelMeeting,
    teachers: tx.teacher,
    levelMeetingTeachers: tx.levelMeetingTeacher,
    users: tx.user,
